...
if (isset($_GET['patid'])) {
    ...
    // Make sure you're using the correct DB connection variable
    $sqlpatient = "SELECT * FROM patient WHERE patientid='$patid'";
    $qsqlpatient = mysqli_query($conn, $sqlpatient); // Changed from $con to $conn

    if ($rspatient = mysqli_fetch_array($qsqlpatient)) {
        echo htmlspecialchars($rspatient['patientname']) . " (Patient ID - " . htmlspecialchars($rspatient['patientid']) . ")";
        echo "<input type='hidden' name='select4' value='" . htmlspecialchars($rspatient['patientid']) . "'>";
    } else {
        echo "<div class='text-danger'>Patient not found.</div>";
    }
}
?>
...
<div class="form-group row">
        ...
        <select class="form-control" name="patient" id="patient" required>
            ...
            <?php
                ...
                while ($rspatient = mysqli_fetch_array($qsqlpatient)) {
                    $selected = (isset($rsedit) && $rspatient['patientid'] == $rsedit['patientid']) ? "selected" : "";
                    echo "<option value='{$rspatient['patientid']}' $selected>
                            {$rspatient['patientid']} - {$rspatient['patientname']}
                          </option>";
                }
            ?>
        </select>
        ...
        <select class="form-control" name="department" id="department" required>
            ...
            <?php
                ...
                while ($rsdepartment = mysqli_fetch_array($qsqldepartment)) {
                    $selected = (isset($rsedit) && $rsdepartment['departmentid'] == $rsedit['departmentid']) ? "selected" : "";
                    echo "<option value='{$rsdepartment['departmentid']}' $selected>
                            {$rsdepartment['departmentname']}
                          </option>";
                }
            ?>
        </select>
        ...
        <select name="doctor" class="form-control" required>
            ...
            <?php
                ...
                while ($rsdoctor = mysqli_fetch_array($qsqldoctor)) {
                    $selected = "";
                    if (isset($_GET['editid']) && isset($rsedit['doctorid']) && $rsdoctor['doctorid'] == $rsedit['doctorid']) {
                        $selected = "selected";
                    }
                    echo "<option value='{$rsdoctor['doctorid']}' $selected>
                          {$rsdoctor['doctorname']} ({$rsdoctor['departmentname']})
                          </option>";
                }
            ?>
        </select>
    ...
    <div class="col-sm-4">
        <select name="status" id="status" class="form-control" required>
            <option value="">-- Select One --</option>
            <option value="Active" <?php if (isset($rsedit['status']) && $rsedit['status'] == 'Active') echo 'selected'; ?>>Active</option>
            <option value="Inactive" <?php if (isset($rsedit['status']) && $rsedit['status'] == 'Inactive') echo 'selected'; ?>>Inactive</option>
        </select>
        <span class="messages"></span>
    </div>
</div>
